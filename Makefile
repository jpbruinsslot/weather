...
build:
	@ echo "+ $@"
	@ CGO_ENABLED=0 go build -a -installsuffix cgo -o ./bin/weather ./cmd/weather

build-linux:
	@ echo "+ $@"
	@ CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -installsuffix cgo -o ./bin/weather-linux-amd64 ./cmd/weather

build-darwin:
	@ echo "+ $@"
	@ CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -a -installsuffix cgo -o ./bin/weather-darwin-amd64 ./cmd/weather

build-windows:
	@ echo "+ $@"
	@ CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -a -installsuffix cgo -o ./bin/weather-windows-amd64 ./cmd/weather
